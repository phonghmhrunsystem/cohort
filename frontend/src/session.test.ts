...
test("the Teacher gradebook route remains within the Teacher workspace", () => {
  expect(canAccess("/teacher/classes/3/gradebook", "TEACHER")).toBe(true);
  expect(canAccess("/teacher/classes/3/gradebook", "STUDENT")).toBe(false);
});
