...
import { Class, Student, getClass, listEnrolledStudents, listStudentCandidates, replaceEnrollment } from "../classes";
...
export function AdminClassPage() {
  ...
  async function searchStudents(value: string) {
    ...
    try {
      const next = await listStudentCandidates(classId, value);
  ...
  async function openRoster() {
    ...
    try {
      const [roster, candidates] = await Promise.all([listEnrolledStudents(classId), listStudentCandidates(classId)]);
  ...
  return <><header className="d-flex justify-content-between gap-3 mb-4"><div><a href="/admin/classes">Classes</a><h1 className="h2 mt-2">{class_.name}</h1><p className="text-secondary">{class_.description || "No description."}</p></div><a className="btn btn-outline-primary align-self-start" href={`/admin/classes?edit=${class_.id}`}>Edit Class</a></header>{error && <div className="alert alert-danger" role="alert">{error}</div>}<section className="card border-0 shadow-sm"><div className="card-body"><div className="d-flex justify-content-between gap-3 mb-3"><h2 ref={studentsHeading} tabIndex={-1} className="h4 mb-0">Students</h2><button className="btn btn-primary btn-sm" onClick={openRoster}>Edit roster</button></div><label className="form-label w-100">Search enrolled Students<input className="form-control" value={query} onChange={(event) => setQuery(event.target.value)} /></label>{students.length === 0 ? <p className="text-secondary mb-0">No enrolled Students match this search.</p> : <ul className="list-group list-group-flush">{students.map((student) => <li className="list-group-item" key={student.id}>{displayName(student)}</li>)}</ul>}</div></section><AppDialog open={dialog} title="Edit roster" pending={saving} fallbackFocus={studentsHeading} onClose={closeRoster} formId={formId} submitLabel={saving ? "Saving…" : "Save roster"} submitDisabled={rosterLoading}><form id={formId} onSubmit={saveRoster}>{error && <div className="alert alert-danger" role="alert">{error}</div>}<label className="form-label w-100">Search Students<input className="form-control" type="search" value={studentQuery} disabled={rosterLoading || saving} onChange={(event) => void searchStudents(event.target.value)} /></label><div className="list-group">{candidates.map((student) => <label className="list-group-item" key={student.id}><input className="form-check-input me-2" type="checkbox" value={student.id} checked={studentIds.includes(student.id)} onChange={() => toggleStudent(student.id)} />{displayName(student)}</label>)}</div></form></AppDialog></>;
}
