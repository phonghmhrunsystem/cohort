...
export function ProfilePage() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const next = await updateProfile({ ...draft, full_name: draft.full_name.trim(), phone: draft.phone.trim(), date_of_birth: draft.date_of_birth || null, gender: draft.gender || null, address: draft.address.trim() });
  ...
  async function savePassword(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setSaving(true); setPasswordError(""); setPasswordFields({});
    try {
      await changePassword(password.current_password, password.new_password);
      setPassword(blankPassword); setPasswordOpen(false);
    } catch (response) {
      const failure = response as ApiFailure;
      setPasswordError(failure.detail === "Request failed." ? "" : failure.detail);
      setPasswordFields(failure.fields ?? {});
    } finally { setSaving(false); }
  }
  ...
  const profileError = error || fieldError(fields, "non_field_errors");
  const passwordFormError = passwordError || fieldError(passwordFields, "non_field_errors");
  return <><header className="d-flex justify-content-between align-items-start gap-3 mb-4"><div><h1 className="h2 mb-1">Hồ sơ cá nhân</h1><p className="text-secondary mb-0">{user.email}</p></div><button className="btn btn-outline-primary" type="button" onClick={() => { setPasswordError(""); setPasswordFields({}); setPasswordOpen(true); }}>Change password</button></header>
    <form className="card border-0 shadow-sm" onSubmit={save}><div className="card-body account-form-grid">
      {profileError && <div className="alert alert-danger account-address mb-0" role="alert">{profileError}</div>}
    ...
    <AppDialog open={passwordOpen} title="Change password" pending={saving} onClose={() => setPasswordOpen(false)} formId={passwordFormId} submitLabel={saving ? "Saving…" : "Change password"}><form id={passwordFormId} onSubmit={savePassword}>{passwordFormError && <div className="alert alert-danger" role="alert">{passwordFormError}</div>}<label className="form-label w-100">Current password<input className={`form-control${fieldError(passwordFields, "current_password") ? " is-invalid" : ""}`} name="current_password" type="password" autoComplete="current-password" value={password.current_password} onChange={(event) => setPassword({ ...password, current_password: event.target.value })} required />{fieldError(passwordFields, "current_password") && <span className="invalid-feedback d-block">{fieldError(passwordFields, "current_password")}</span>}</label><label className="form-label w-100">New password<input className={`form-control${fieldError(passwordFields, "new_password") ? " is-invalid" : ""}`} name="new_password" type="password" autoComplete="new-password" value={password.new_password} onChange={(event) => setPassword({ ...password, new_password: event.target.value })} minLength={8} maxLength={128} required />{fieldError(passwordFields, "new_password") && <span className="invalid-feedback d-block">{fieldError(passwordFields, "new_password")}</span>}</label></form></AppDialog>
  </>;
}
