...
const harness = vi.hoisted(() => ({ getClass: vi.fn(), listStudentCandidates: vi.fn(), listEnrolledStudents: vi.fn(), replaceEnrollment: vi.fn() }));
...
beforeEach(async () => {
  ...
  harness.listStudentCandidates.mockResolvedValue([enrolled, candidate]);
...
test("editing the roster searches active Students, prechecks members, saves once, and preserves a rejected draft", async () => {
  ...
  harness.listStudentCandidates.mockReset()
  ...
  expect(harness.listStudentCandidates).toHaveBeenLastCalledWith(4, "Linh");
...
test("editing after a roster search retains members outside the page filter", async () => {
  ...
  harness.listStudentCandidates.mockResolvedValue([enrolled, hiddenMember, candidate]);
