...
const teacherName = (teachers: User[], id: number) => {
  const teacher = teachers.find((candidate) => candidate.id === id);
  return teacher ? displayName(teacher) : "Unknown teacher";
};
...
export function AdminClassesPage() {
  ...
  return <>
    ...
    {loading ? <div className="alert alert-secondary">Loading Classes…</div> : classes.length === 0 ? <div className="alert alert-secondary">No Classes match this search.</div> : <section aria-label="Classes"><ul className="list-group">{classes.map((class_) => <li className="list-group-item d-flex justify-content-between align-items-start gap-3" key={class_.id}>
      <div className="min-w-0">
        <h2 className="h5 mb-1 text-break">{class_.name}</h2>
        <p className="text-secondary text-break mb-1">{class_.description || "No description."}</p>
        <p className="small mb-0">Teacher: {teacherName(teachers, class_.teacher_id)} · {localTime(class_.starts_at)} – {localTime(class_.ends_at)}</p>
      </div>
      <div className="d-flex flex-wrap align-items-start justify-content-end gap-2">
        <button className="btn btn-outline-primary btn-sm" onClick={() => open(class_)}>Edit</button>
        <a className="btn btn-primary btn-sm" href={`/admin/classes/${class_.id}`}>Open Class</a>
      </div>
    </li>)}</ul></section>}
    <AppDialog open={dialog} title={editing ? "Edit Class" : "Create Class"} pending={saving} onClose={() => setDialog(false)} formId={formId} submitLabel={saving ? "Saving…" : "Save Class"}><form id={formId} onSubmit={save}>{error && <div className="alert alert-danger" role="alert">{error}</div>}<label className="form-label w-100">Name<input className="form-control" value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} minLength={2} maxLength={100} required /></label><label className="form-label w-100">Description<textarea className="form-control" value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} maxLength={1000} /></label><label className="form-label w-100">Teacher<select className="form-select" value={draft.teacher_id} disabled={!!editing} onChange={(event) => setDraft({ ...draft, teacher_id: Number(event.target.value) })} required><option value="" disabled>Choose Teacher</option>{teachers.map((teacher) => <option value={teacher.id} key={teacher.id}>{displayName(teacher)}</option>)}</select></label><label className="form-label w-100">Starts<input className="form-control" type="datetime-local" step="1" value={draft.starts_at} onChange={(event) => setDraft({ ...draft, starts_at: event.target.value })} required /></label><label className="form-label w-100">Ends<input className="form-control" type="datetime-local" step="1" value={draft.ends_at} onChange={(event) => setDraft({ ...draft, ends_at: event.target.value })} required /></label></form></AppDialog>
  </>;
}
