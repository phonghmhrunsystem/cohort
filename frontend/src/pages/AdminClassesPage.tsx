...
export function AdminClassesPage() {
  ...
  async function load() {
    setLoading(true);
    try { const [next, activeTeachers] = await Promise.all([listClasses(query), listTeachers()]); setClasses(next); setTeachers(activeTeachers); }
    catch (response) { setError(message(response)); }
    finally { setLoading(false); }
  }
  function open(class_?: Class) {
    setEditing(class_ ?? null);
    setDraft(class_ ? { ...class_, teacher_id: class_.teacher_id, starts_at: class_.starts_at.slice(0, 16), ends_at: class_.ends_at.slice(0, 16) } : emptyDraft);
    setError(""); setDialog(true);
  }
  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true); setError("");
    try {
      const payload: ClassDraft = { name: draft.name.trim(), description: draft.description.trim(), starts_at: draft.starts_at, ends_at: draft.ends_at };
