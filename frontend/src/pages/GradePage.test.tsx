...
const submission = { id: 5, assignment_id: 1, student_id: 2, version: 1, original_filename: "essay.pdf", created_at: "2026-07-27T00:00:00Z", graded: false };
...
test("rubric assignment renders one required number input per criterion", () => {
  const html = render([rubricAssignment, submission, "Nguyen Van A", { 10: 0, 11: 0 }, 0, "", null, "", false]);
...
test("assignment with no rubric renders a single manual total input", () => {
  const html = render([manualAssignment, submission, "Nguyen Van A", {}, 0, "", null, "", false]);
...
test("after a grade is saved the confirmation replaces the form", () => {
  ...
  const html = render([rubricAssignment, submission, "Nguyen Van A", { 10: 0, 11: 0 }, 0, "", grade, "", false]);
...
test("a submission that is already graded shows Đã chấm instead of the grading form", () => {
  const html = render([rubricAssignment, { ...submission, graded: true }, "Nguyen Van A", { 10: 0, 11: 0 }, 0, "", null, "", false]);
