...
import { Class, ClassRoster, StudentProfile, getClass, getClassStudent, listClassStudents } from "../classes";
...
export function TeacherClassPage() {
  ...
  const [dialog, setDialog] = useState<"assignment" | "rubric" | "profile" | null>(null);
  ...
  const assignmentFormId = useId();
  const rubricFormId = useId();

  useEffect(() => { void load(); }, [studentQuery]);
  async function load() {
    try {
      const [next, nextRoster, coursework] = await Promise.all([getClass(id), listClassStudents(id, studentQuery), listAssignments(id)]);
      setClass(next); setRoster(nextRoster); setAssignments(coursework);
    } catch (response) { setError(message(response)); }
  }
  async function openProfile(studentId: number) {
    setError(""); setDialog("profile"); setProfile(null);
    try { setProfile(await getClassStudent(id, studentId)); }
    catch (response) { setError(message(response)); setDialog(null); }
  }
  ...
  return <>
    ...
    <div className="teacher-tabs mb-3" role="tablist" aria-label="Class content"><button className={`btn ${tab === "students" ? "btn-primary" : "btn-outline-primary"}`} onClick={() => selectTab("students")} type="button">Students</button><button className={`btn ${tab === "assignments" ? "btn-primary" : "btn-outline-primary"}`} onClick={() => selectTab("assignments")} type="button">Assignments</button><a className="btn btn-outline-primary" href={`/teacher/classes/${id}/gradebook`}>Bảng điểm</a></div>
    {tab === "students" ? <section className="card border-0 shadow-sm"><div className="card-body"><h2 className="h4">Students</h2><label className="form-label w-100">Search enrolled Students<input className="form-control" value={studentQuery} onChange={(event) => setStudentQuery(event.target.value)} /></label>{roster && <p className="text-secondary">Đã ghi danh {roster.enrolled_students} · Đã nộp {roster.submitted_students} · Đã chấm {roster.graded_students}</p>}{roster?.students.length ? <ul className="list-group list-group-flush">{roster.students.map((student) => <li className="list-group-item d-flex justify-content-between align-items-center gap-2" key={student.id}><span>{displayName(student)}<br /><small className="text-secondary">Nộp bài: {student.submitted_assignments} / {roster.total_assignments} · Chấm điểm: {student.graded_assignments} / {roster.total_assignments}</small></span><button className="btn btn-outline-primary btn-sm" type="button" aria-label={`Xem hồ sơ của ${displayName(student)}`} onClick={() => void openProfile(student.id)}>Xem hồ sơ</button></li>)}</ul> : <p className="text-secondary mb-0">No enrolled Students match this search.</p>}</div></section> : <section><div className="d-flex justify-content-between align-items-center gap-3 mb-3"><h2 className="h4 mb-0">Assignments</h2><button className="btn btn-primary" onClick={() => openAssignment()}>Create Assignment</button></div>{assignments.length ? <div className="account-grid">{assignments.map((assignment) => <article className="card border-0 shadow-sm" key={assignment.id}><div className="card-body"><h3 className="h5">{assignment.title}</h3><p className="text-secondary">{assignment.description}</p><p className="small">Due {new Date(assignment.due_at).toLocaleString()}</p><div className="d-flex gap-2"><button className="btn btn-outline-primary btn-sm" onClick={() => openAssignment(assignment)}>Edit</button><button className="btn btn-outline-primary btn-sm" onClick={() => openRubric(assignment)}>Edit rubric</button><a className="btn btn-outline-primary btn-sm" href={`/teacher/assignments/${assignment.id}`}>View submissions</a></div></div></article>)}</div> : <div className="alert alert-secondary">No assignments yet.</div>}</section>}
    <AppDialog open={dialog === "assignment"} title={editing ? "Edit Assignment" : "Create Assignment"} pending={saving} onClose={() => setDialog(null)} formId={assignmentFormId} submitLabel={saving ? "Saving…" : "Save Assignment"}><form id={assignmentFormId} onSubmit={saveAssignment}>{error && <div className="alert alert-danger" role="alert">{error}</div>}<label className="form-label w-100">Title<input className="form-control" value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} minLength={2} maxLength={150} required /></label><label className="form-label w-100">Description<textarea className="form-control" value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} minLength={10} maxLength={5000} required /></label><label className="form-label w-100">Due date<input className="form-control" type="datetime-local" step="0.001" value={draft.due_at} onChange={(event) => setDraft({ ...draft, due_at: event.target.value })} required /></label></form></AppDialog>
    <AppDialog open={dialog === "rubric"} title="Edit rubric" pending={saving} onClose={() => setDialog(null)} formId={rubricFormId} submitLabel={saving ? "Saving…" : "Save rubric"} submitDisabled={total !== 100}><form id={rubricFormId} onSubmit={saveRubric}>{error && <div className="alert alert-danger" role="alert">{error}</div>}<p className={total === 100 ? "text-success" : "text-danger"}>Total: {total} / 100</p>{criteria.map((criterion, index) => <div className="rubric-row mb-2" key={criterion.id ?? index}><label className="form-label">Criterion<input className="form-control" value={criterion.title} onChange={(event) => setCriteria(criteria.map((item, i) => i === index ? { ...item, title: event.target.value } : item))} required /></label><label className="form-label">Points<input className="form-control" type="number" min="1" max="100" value={criterion.maximum_score} onChange={(event) => setCriteria(criteria.map((item, i) => i === index ? { ...item, maximum_score: Number(event.target.value) } : item))} required /></label><button className="btn btn-outline-danger btn-sm align-self-end" type="button" aria-label={`Xóa ${criterion.title.trim() || `tiêu chí ${index + 1}`}`} onClick={() => setRemovingCriterion(criterion)}>Xóa</button></div>)}<button ref={addCriterionButton} className="btn btn-outline-primary btn-sm mb-3" type="button" onClick={() => setCriteria([...criteria, { title: "", maximum_score: 1 }])}>Add criterion</button></form></AppDialog>
    ...
    <AppDialog open={dialog === "profile"} title={profile ? displayName(profile) : "Hồ sơ"} onClose={() => setDialog(null)}>{!profile ? <p className="text-secondary mb-0">Đang tải…</p> : <dl className="mb-0">
      <dt>Email</dt><dd>{profile.email}</dd>
      <dt>Điện thoại</dt><dd>{profile.phone || "—"}</dd>
      <dt>Ngày sinh</dt><dd>{profile.date_of_birth || "—"}</dd>
      <dt>Giới tính</dt><dd>{profile.gender ? genderLabel[profile.gender] : "—"}</dd>
      <dt>Địa chỉ</dt><dd>{profile.address || "—"}</dd>
      <dt>Tiến độ</dt><dd>Nộp bài: {profile.submitted_assignments} / {profile.total_assignments} · Chấm điểm: {profile.graded_assignments} / {profile.total_assignments}</dd>
      <dt>Lớp học chung</dt><dd>{profile.shared_classes.length ? profile.shared_classes.map((shared) => shared.name).join(", ") : "—"}</dd>
    </dl>}</AppDialog>
  </>;
}
