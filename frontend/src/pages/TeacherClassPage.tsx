...
const localDue = (value: string) => value ? new Date(value).toISOString().slice(0, 16) : "";
...
export function TeacherClassPage() {
  ...
  function openAssignment(assignment?: Assignment) {
    ...
    setDraft(assignment ? { title: assignment.title, description: assignment.description, due_at: localDue(assignment.due_at) } : emptyDraft);
  ...
  async function saveAssignment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const value = { ...draft, title: draft.title.trim(), description: draft.description.trim(), due_at: new Date(draft.due_at).toISOString() };
