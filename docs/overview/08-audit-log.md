...
- **Class resource creation** — `ClassResourcesView.post` calls `create_notifications` but never `write_audit`, unlike `ClassAssignmentsView.post` right next to it. This one is an inconsistency, not a decision: a teacher can publish a link to the whole Class with no trace of who or when. One `write_audit(action="class_resource.created", ...)` call inside the existing `transaction.atomic()` closes it. Flagged in [07 §2.3](07-notifications-and-resources.md#23-teacher-side-resource-management) too.
