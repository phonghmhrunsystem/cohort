...
- **Class resource writes — creation, edit and delete** — `ClassResourcesView.post` calls `create_notifications` but never `write_audit`, unlike `ClassAssignmentsView.post` right next to it, and `ClassResourceDetailView` writes nothing either. This one is an inconsistency, not a decision: a teacher can publish a link to the whole Class, or delete a document off disk, with no trace of who or when. Three `write_audit` calls (`class_resource.created` / `.updated` / `.deleted`) inside the existing transactions close it. Auditing only the delete would leave this table's coverage arbitrary — either all three or none. Flagged in [07 §2.3](07-notifications-and-resources.md#23-teacher-side-resource-management) too.
